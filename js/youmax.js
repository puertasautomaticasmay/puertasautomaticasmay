...
(function ($) {
	...
	showPlaylists = function(response,loadMoreFlag) {
		...
		for(var i=0; i<playlistArray.length; i++) {
			...
			if((i+youmax_global_options.youmaxItemCount-zeroPlaylistCompensation)%youmaxColumns!=0)
				$('#youmax-video-list-div').append('<div class="youmax-video-tnail-box" style="width:'+((100/youmaxColumns)-4)+'%;" id="'+playListId+'"><div class="youmax-video-tnail" style="filter: progid:DXImageTransform.Microsoft.AlphaImageLoader( src=\''+playlistThumbnail+'\', sizingMethod=\'scale\'); background-image:url(\''+playlistThumbnail+'\')"><div class="youmax-playlist-sidebar" id="youmax-playlist-sidebar-'+playListId+'"><span class="youmax-playlist-video-count"><b>'+playlistSize+'</b><br/>VIDEOS</span></div></div><span class="youmax-video-list-title">'+playlistTitle+'</span><br/><span class="youmax-video-list-views">Hace '+getDateDiff(playlistUploaded)+'</span></div>');
			else
	...
	showUploads = function(response,playlistTitle,loadMoreFlag) {
		...
		for(var i=0; i<uploadsArray.length; i++) {
			...
			if((i+youmax_global_options.youmaxItemCount)%youmaxColumns!=0)
				$('#youmax-video-list-div').append('<div class="youmax-video-tnail-box" style="width:'+((100/youmaxColumns)-4)+'%;" id="'+videoId+'"><div class="youmax-video-tnail" style="filter: progid:DXImageTransform.Microsoft.AlphaImageLoader( src=\''+videoThumbnail+'\', sizingMethod=\'scale\'); background-image:url(\''+videoThumbnail+'\')"><div class="youmax-duration"></div></div><span class="youmax-video-list-title">'+videoTitle+'</span><br/><span class="youmax-video-list-views">Hace '+getDateDiff(videoUploaded)+'</span></div>');
			else
	...
	displayVideoStats = function(response) {
		...
		for(var i=0; i<videoArray.length; i++) {
			...
			$videoThumbnail.find('.youmax-video-list-views').prepend(videoViewCount+' vistas | ');
	...
	resetLoadMoreButton = function() {
		...
		$youmaxLoadMoreDiv.text('Ver Mas');
	},
